...
def createTicket():
    ...
    mainLabel= Label[gameWindow, width=65, height=50,relief='edge',borderWidth=5]
    ...
    for row in range(0,3):
        rowList=[]
        boxButton=Button[gameWindow,borderWidth=30,pady=40,padx=30]
        boxButton.place(x=xpos,y=ypos)
        rowList.append(boxButton)
    ticketGrid.append(boxButton)
...
def gameWindow():
    ...
    bg = ImageTk.PhotoImage(file = "./bg.jpeg")
    ...
    # Add Text
    canvas2.create_text( screen_width/2, screen_height/5, text = "Tambola", font=("Chalkboard SE",100), fill="white")
